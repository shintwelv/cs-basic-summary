...
순간순간 가장 적은 비용의 색을 선택하는 그리디(Greedy) 방식은 미래의 선택에 악영향을 미칠 수 있어 적합하지 않다. 반면, $i$번째 집의 색이 결정되었을 때 이전 $i-1$번째 집의 상태에만 영향을 받는 **최적 부분 구조(Optimal Substructure)** 를 가지므로 **[동적 계획법(Dynamic Programming, DP)](../dp.md)** 으로 접근하는 것이 가장 효율적이다.
